...
- **狀態**：📋 待處理
- **優先度**：中
- **描述**：Dashboard 執行中任務 banner 目前僅顯示任務名稱，建議加入從開始計時至今的累計時間（每秒更新）。
- **影響範圍**：`TimeTracker.tsx` — 使用 `setInterval` 每秒重算 elapsed time
...
- **狀態**：💡 建議
- **優先度**：中
- **描述**：統計報表除了圓餅圖，補充「當週每日工時長條圖」（Recharts BarChart），可直觀看出哪天工作量最重。
- **影響範圍**：`Stats.tsx`（新增 BarChart section）
...
- **狀態**：💡 建議
- **優先度**：低
- **描述**：顯示過去 N 週每週總工時的折線圖，方便追蹤工作節奏與工作量趨勢。
- **影響範圍**：`Stats.tsx` 或新增頁面
...
- **狀態**：📋 待處理
- **優先度**：低
- **描述**：`npm run build` 顯示 `index.js` 超過 500 KB（目前約 1,183 KB）。雖不影響功能，但可透過 code-splitting 改善載入效能。
