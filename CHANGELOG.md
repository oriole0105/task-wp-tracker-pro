...
第一個正式版本。涵蓋完整的任務規劃、工時紀錄、週報生成與跨裝置同步功能。

**核心功能：**
- 任務管理：WBS 5 層父子架構、狀態追蹤、封存庫、任務複製、批次 JSON 匯入
- 排程視圖：Outlook 式垂直時間軸、ICS 匯入/匯出、Timeslot 快速新增
- 週報生成：PlantUML WBS / 甘特圖 / 進度追蹤表（有進展 / 無進展）、AsciiDoc 匯出、SPI 時程績效
- 統計報表：任務狀態環形圖 + 工時圓餅圖、任務節點篩選
- 工作產出追蹤：產出類型管理、完成度快照、歷史趨勢折線圖
- 任務里程碑：甘特圖菱形標記、8 色選擇
- 跨裝置同步：差異匯出 + 智慧合併匯入
- PWA：離線可用、可安裝至手機主畫面
- 手機響應式：底部導航列、全螢幕 Dialog、FAB 快速新增
- 人員名單管理、設定備份/還原、任務清單 CSV 匯出
...
- **待辦事項頁面（快速記錄）**
  - 新增 `/todos` 頁面，供使用者快速記錄短期個人待辦小事（非正式任務，不納入工時追蹤）
  - 欄位：勾選完成、事項描述（必填）、開始日期（自動產生）、完成日期（勾選時自動填入）
  - 已完成項目自動收入底部「已完成」摺疊區，保持輸入介面簡潔
  - 已完成區提供「匯出 .txt」與「清除全部」功能
  - 頁面頂部附說明提示，引導使用者區分待辦事項與正式任務的適用情境

- **事件時間軸**
  - 任務編輯對話框新增「事件時間軸」摺疊區（位於里程碑與工作產出之間）
  - 每個任務可記錄多筆「日期 ＋ 事件描述」，隨任務一起儲存
  - 新增 `/task-timeline` 頁面：選取有事件記錄的任務後，以垂直時間軸呈現其所有事件

### 說明文件

- 使用者手冊新增「待辦事項」與「事件時間軸」章節說明
- 導覽列對照表更新（新增待辦事項 `/todos`、事件軸 `/task-timeline`）
- 附錄資料結構表新增 `TodoItem`、`TaskTimelineEntry`

- **週報生成預設顯示上週**
  - 週報頁 `reportAnchorDate` 初始值改為智慧判斷：週一開啟時自動定位至上週，其餘日子維持當週
  - 方便週一時直接生成上週週報，不需再手動往前切換

- **任務複製後綴改為流水號**
  - 複製任務時，原本的「（副本）」後綴改為 `-1`、`-2`、`-3` 流水號格式
  - 開啟「防止任務同名」設定時，自動剝除既有 `-N` 尾綴後從最小可用號碼開始；關閉時一律加 `-1`

- **系統設定：防止任務同名**
  - 系統設定頁新增「行為設定」區塊，含「防止任務同名」開關（預設開啟）
  - 開啟後複製任務時自動選用不與現有任務衝突的流水號，解決 PlantUML 同名節點問題
  - 設定值持久化至 localStorage

- **設定模式任務名稱 inline 編輯**
  - 任務管理頁設定模式下，任務名稱欄改為可直接編輯的 TextField
  - Enter 或 blur 觸發儲存；Escape 放棄編輯還原原名；空白輸入自動還原
  - 若「防止任務同名」設定開啟，重複名稱時顯示紅色錯誤提示並阻止儲存
...
- **手機底部導航列（階段 1：響應式導航）**
  ...
  - 5 個項目：時間軸 / 任務 / ＋（快速新增 FAB）/ 產出 / 更多
  - 「＋」按鈕凸出設計，點擊後可選擇新增時段或新增任務
...
- **跨裝置資料同步（階段 2：智慧合併）**
  ...
  - **差異匯出**：可選擇匯出近 N 天內新建或修改的 tasks + timeslots（預設 7 天）
  - **智慧合併匯入**：匯入時同 ID 資料比對 `updatedAt` 取較新版本，新 ID 直接加入
  - 系統設定頁新增「跨裝置資料同步」區塊，含差異匯出 + 合併匯入按鈕

- **Dialog / 表單響應式（階段 3）**
  - TaskForm：手機端全螢幕 Dialog + AppBar 含關閉按鈕
  - TimeTracker 編輯/新增時間紀錄 Dialog：手機端全螢幕
  - 確認框、匯出等小型 Dialog 維持原尺寸不受影響
...
- **甘特圖邏輯修正**
  ...
  - 里程碑改為緊接在所屬任務 bar 之後輸出，不再集中排列於所有任務之後；`section` 任務的里程碑接在 section header 之後；無日期 `section` 任務（不在 `ganttActiveTasks` 內）的里程碑仍以 fallback 方式輸出於末尾
  - 修正 `section` 任務無日期時不出現於甘特圖的 bug：`ganttActiveTasks` 過濾條件改為 `section` 模式任務無條件放行，不再要求有日期或 timeslot

- **任務列表設定模式**
  - 工具列新增「設定模式」切換按鈕（`Tune` 圖示），啟用時以橙色高亮顯示
  - 設定模式下表格欄位替換為可直接編輯的控制元件，變更即時寫入 store
  - 支援的設定項目：WBS 顯示（Switch）、甘特圖模式（下拉：進度列/章節標題/不顯示）、預估開始日期（date input）、預估完成日期（date input）、任務分類（下拉）、狀態（下拉）、追蹤完成度（Switch）、完成度 %（0–100 數字輸入，追蹤完成度關閉時 disabled）
  - 頂部顯示提示 banner，提醒變更即時儲存，可使用 Undo 還原

- **TaskForm 解除章節標題強制填日期限制**
  - 甘特圖模式設為「章節標題」時，預估開始/完成日期改為選填（原本與「進度列」同樣強制必填）
  - 日期欄位星號與錯誤提示改為僅在「進度列」模式下顯示

- **Timeslot 時間重疊防護**
  - 新增/編輯 Timeslot 時自動偵測時間重疊，有衝突即阻擋儲存
  - 錯誤訊息顯示衝突的任務名稱與時間範圍（頂部 Snackbar 提示）
  - Dialog 保持開啟，使用者可直接調整時間後重試
  - ICS 匯入時逐筆檢查重疊，自動跳過衝突筆數並提示統計結果
  - 新增 `src/utils/timeslotOverlap.ts` 重疊偵測工具函式
